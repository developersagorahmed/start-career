...
const JobDetails = () => {
    ...
    const {companyName,remoteOrOnsite,fullTimeOrPartTime,companyLogo,jobDescription,jobResponsibility,educationalRequirements,experiences,salary,jobTitle,phone,email,location}=mainData
    const addToDb = (id,companyName,remoteOrOnsite,fullTimeOrPartTime,companyLogo,jobDescription,jobResponsibility,educationalRequirements,experiences,salary,jobTitle,phone,email,location) => {
        const preBook=JSON.parse(localStorage.getItem('id'))
        const data={id,companyName,remoteOrOnsite,fullTimeOrPartTime,companyLogo,jobDescription,jobResponsibility,educationalRequirements,experiences,salary,jobTitle,phone,email,location}
        ...
        <div>
          <div className='bobDetails mt-[130px]'>
           ...
           <div className='w-[450px] h-full pb-[60px] bg-gradient-to-r from-[#7E90FE1A] to-[#9873FF1A] rounded-lg p-4'>
            ...
            <button onClick={()=>addToDb(id,jobTitle,companyLogo,companyName,location,salary,remoteOrOnsite,fullTimeOrPartTime)} className='mt-7 bg-gradient-to-r from-[#7E90FE] to-[#9873FF] w-[450px] h-[65px] rounded-lg text-xl font-extrabold text-white'   >Apply Now</button>
           </div>
          </div>
